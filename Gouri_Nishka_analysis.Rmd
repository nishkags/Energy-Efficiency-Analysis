...
- I made a histogram comparing the Heating Load and Orientation and found that there is no clear relationship between them despite expectations that building orientation would influence heating efficiency due to sun exposure; there was no clustering or directional advantage. Since orientation adds no predictive value, I'm going to be discluding it from the final regression model.
...
- Wall Area was quite scattered with clustering aaround the middle, but with a fairly positive linear relationship.A smaller wall area (250-300 m^2) will lead to less energy usage, while a higher one (>400), will lead to more energy usage. 
...
- There were only two height values, 3.5 and 7.0, and the relationship is positive and linear so thattaller buildings needed more heating
...
- For Glazing Area Distribution, I made a density plot. Group 0, or no windows, peaks far to the left, meaning it has a lower heating load, but groups 1-5 have similar shapes, meaning they don't contribute much unique information on thier own, which is why I collapsed it into a binary variable (none vs. present).
...
#Model Training
I chose to use a multiple linear regression model because it is an interpretable model that works well when the relationship between predictors and the outcome is approximately linear. It allows us to understand how each feature affects Heating Load. The model was trained using a 75/25 train-test splot and I used a 5-fold cross-validation to evaluate performance. 
```{r}
library(tidymodels)
lm_model <- linear_reg() %>%
  set_engine("lm")
...
Evaluating the RMSE, MAE, and R^2 metrics, I found that the model shows strong performance indicated by a mae of just 1.09 and rmse of 1.64, a low standard error rate, and a high R^2 value. The model explains 97.2% ofo the variance in Heating Load. I used the model to make predictions on unseen data, and found that relative compactness (positive effect), overall height (negative effect), and the engineered surface height (positive effect) interaction terms were most influential. Larger buildings with greater surface area x height tend to require more heating energy. Interestingly, I also found that taller overall height uses less heating, possibly due to internal heat distribution. Surprisingly, as buildings are more compact, they also require more heating. This could be due to interaction with other variables. Surface area and glazing area and glazing type were also moderately or weakly strong predictors was slightly week, more surface area more windows increase heating load slightly. Wall area was not statistically significant, and the model dropped roof area due to collinearity.
